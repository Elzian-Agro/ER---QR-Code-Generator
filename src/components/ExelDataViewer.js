      ...
      <table className="custom-table">
        <thead>
          {excelData.length ? (
            <tr>
              {isLocalFile ? (
                excelData[0].map((columnName, index) => <th key={index}>{columnName}</th>)
              ) : (
                  Object.keys(excelData[0]).slice(1).map((columnName, index) => (
         ...
         <tbody>
          {Object.values(excelData).slice(1).map((info, rowIndex) => (
            <tr key={rowIndex}>
              {isLocalFile ? (
                ...
                Object.values(info).slice(1).map((value, colIndex) => (
                  <td key={colIndex}>
                    {colIndex === 4 ? formatDateFromExcel(value) : colIndex === 5 ? decodeEntities(value) : value}
                  </td>
                ))
              )}
            </tr>
